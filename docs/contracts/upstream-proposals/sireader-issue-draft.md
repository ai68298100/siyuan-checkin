...
你好！我是思源打卡插件「小驴打卡」（siyuan-checkin）的作者。小驴已通过思阅实现「有效阅读分钟 → 每日打卡」联动，并随 **v18.4.0** 正式发布（消费侧 fallback 已在生产可用）；本帖提议把这个集成面**版本化、契约化**，让双方长期稳定协作。
...
- 消费侧现状：随 v18.4.0 发布（opt-in 默认关），通过隔离真实内核 e2e 回归（含「思阅生命周期事件在真实内核累计并幂等写入」用例）。
...
如果方向可行，命名、字段、事件名都可按思阅的习惯调整，小驴侧跟进适配；需要的话我愿意以 PR 形式协助补充契约文档与测试。完整草案（含机器可读 JSON 契约与消费方实现说明）见：https://github.com/ai68298100/siyuan-checkin/blob/main/docs/upstream-api-proposals-2026-09.md 。谢谢！
