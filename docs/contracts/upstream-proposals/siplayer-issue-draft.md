...
你好！我是思源打卡插件「小驴打卡」（siyuan-checkin）的作者。小驴正通过思媒播放器实现「有效观看分钟 → 每日打卡」联动，并随 **v18.4.0** 正式发布（消费侧采样 fallback 已在生产可用）；本帖提议把这个集成面**版本化、契约化**，让双方协作长期稳定。
...
- 联动保持 **opt-in 默认关**（实验标注）；上游契约发布并通过真实宿主验收后，canonical source 才切上游口径，本地采样降级为诊断比对——两路永不双重累计；
- 隐私边界：不要求暴露 ArtPlayer 私有对象或内部存储；媒体标题/路径不进打卡事件；
- 消费侧现状：随 v18.4.0 发布（opt-in 默认关），通过隔离真实内核 e2e 回归（含「思播 controller 采样在真实内核累计并幂等写入」用例）。
...
如果方向可行，事件名、字段、能力命名都可按思播的习惯调整；需要的话我愿意以 PR 形式协助补契约文档与消费端测试。完整草案（含机器可读 JSON 契约与消费方实现说明）见：https://github.com/ai68298100/siyuan-checkin/blob/main/docs/upstream-api-proposals-2026-09.md 。谢谢！
