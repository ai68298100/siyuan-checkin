...
| R-50.1 | 纯建议链可自动推进 | 草案、差异、缓存、冲突、审计和撤销测试 | 思源智能体入口、真实模型、宿主复制/返回 |
...
R-A0～R-A12 是执行泳道标签，用来映射前面已有 R-00/R-10/R-20/R-30/R-40/R-50 批次和横切基础设施，不是重复立项。每条泳道都先做本地可验证部分，宿主或外部协作证据单独保留开放；在不要求用户操作、不请求外部账号、不改变用户工作区数据的前提下，按 A0→A1→A7→A2→A11→A12→A8→A3→A4→A5→A9/A10→A6 推进（与第五节开工顺序一致）。
...
4. 受控智能体（R-50.1）的纯建议链——证据引用、差异预览、冲突指纹、失效缓存和撤销——并入本泳道自动验证；真实模型与宿主入口保持 host-pending。
