...
### 第 4 批：治理、互操作与收尾（R-A18，2026-09-26 部分交付，T-1468）

1. **R-18.1 分享图片导出** ✅：年度格子+统计本地 canvas PNG（features/share-card.ts 布局模型+绘制分离），回顾工具「分享图」入口，原生容器感知保存通道；零网络零遥测。
2. **R-18.4 UI 台账保留词表清理** ✅：loading/success/is-saving/msg.saving 退役（零消费方核查），台账断言翻转为「不得回流」。
3. **R-18.5 里程碑分级庆祝** ✅（D-263 收尾）：连击阶梯命中升级庆祝（金标+加重弹跳），reducedMotion 静态降级。
4. **R-18.2 诊断导出预览深化**（A10 剩余）：待排。
5. **R-18.3 小项收口**：66 天成熟度进度条、周开始日开关、宽屏 bento 布局重排：待排（分别依赖洞察改版与数据聚合确认）。
