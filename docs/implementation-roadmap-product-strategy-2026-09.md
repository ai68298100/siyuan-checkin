...
- **T-1465 问卷式日记打卡**（用户想法，调研见 benchmark 第十六节，D-273 定型）：✅ 已交付——打卡项目绑定问卷模板（内置 5 预设+自建），填完写入今日日记或指定文档并完成打卡；写入走 summary-resident 同款幂等/旁路纪律；真实内核 e2e 22/22。

### 第 3 批：洞察与可视化深化（R-A17，2026-09-26 部分交付，T-1467）

1. **R-17.1 确定性相关性洞察** ✅：最小样本纪律（重叠 ≥4 天且 |r|≥0.4）+滞后一天（严格更强才呈现）；纯统计无 AI 依赖，防相关性误读措辞入 calm 禁则（report.correlationNote）。
...
4. **R-17.4 fractured/sparse 热图**：留待渲染块迭代排批（随热力图改版窗口）。

### 第 4 批：治理、互操作与收尾（R-A18，2026-09-26 交付，T-1468/T-1469）

1. **R-18.1 分享图片导出** ✅：年度格子+统计本地 canvas PNG（features/share-card.ts 布局模型+绘制分离），回顾工具「分享图」入口，原生容器感知保存通道；零网络零遥测。
2. **R-18.2 诊断导出预览深化** ✅：裸 confirm 升级为结构化预览对话框——原因码计数表+时间范围+内容边界披露固定显示，确认后走 versioned 导出通道。
3. **R-18.3 小项收口**：66 天成熟度刻度条 ✅（洞察页，aria progressbar+参考非标准文案）；周开始日开关：**经架构评估正式关闭（2026-09-26）**——周一优先的对齐散布在 rules（周排期求值）、analytics（周区间聚合）、charts/checkin-block（渲染对齐）、checkin-log-hierarchy、insights 共 7 文件 8 处，其中 rules 属共享数据模型的排期语义：做成设备显示偏好会使多端同步的完成窗口语义分叉（两设备偏好不同 → 周排期行为不一致），做进共享模型则是数据语义变更而非显示偏好——收益（习惯应用的常见偏好）不抵语义分叉风险，维持周一优先并留档结论；宽屏 bento 布局重排：待排（内容层稳定后）。
4. **R-18.4 UI 台账保留词表清理** ✅：loading/success/is-saving/msg.saving 退役（零消费方核查），台账断言翻转为「不得回流」。
5. **R-18.5 里程碑分级庆祝** ✅（D-263 收尾）：连击阶梯命中升级庆祝（金标+加重弹跳），reducedMotion 静态降级。
