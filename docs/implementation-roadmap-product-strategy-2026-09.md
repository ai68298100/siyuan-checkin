...
- **T-1461 / R-A13** 低压力呈现与包容性设计基线（色阶冗余编码／双日规则文案／calm 禁则／三通道冗余／44-48px 触控／undo 审查）
- **T-1462 / R-A14** 数值快捷记录预设增量（per-item 步长 chips，编辑器+今日页+today 渲染块）
- **T-1463 / R-A15** 迁移健壮性与边界用例（跨午夜双日归账断言／导入未知字段保留／合并语义参考记录）

### 第 2 批：今日与回顾呈现增强（R-A16）

1. **R-16.1 今日完成度环**：行动台摘要条升级 SVG wrap-around 环（点击展开明细），宽屏 dock 常驻小环；reducedMotion 下静态数字降级；数据全部来自 today-dashboard 既有投影，零新计算口径。
2. **R-16.2 sparkline 报告卡**：回顾报告卡统计值配 7/30 日 sparkline（复用 charts 既有路径数据），压缩移动端卡高；保留展开到完整趋势图的入口。
