...
> 状态：打卡侧 P0 v1 已落地（2026-09-18），可供双方评审。日期摘要 API 与预设模板已就绪，Task Horizon 侧需新增消费层。2026-09-22 新增项目级日历可见性研究，**尚未改 v1 机器契约或运行代码**；详细范围见 [日历可见性规划](roadmap-task-horizon-calendar-visibility-2026-09.md)。
...
### L1.1 项目级日历可见性（T-1389～T-1394，研究完成/未开工）
...
- 现有 `getEventRangeSummary()` 只有日期聚合，无法自行完成项目级过滤；长期优先新增有界项目×日期投影（推荐 `calendar.read` / `getCalendarProjection` 方向），在双方契约冻结前不修改 `task-horizon-v1.json`。
...
- 写：`events.record`（见 L2）；返回事件表示新写入或已存在的幂等结果，返回 `undefined` 才表示非法/拒绝；对重复事件不要生成新 externalRef。
