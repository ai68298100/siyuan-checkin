...
当前 API 版本为 `5`（v4 能力面冻结兼容，只增不删），名称为 `siyuanCheckin`，协议标识为 `siyuan-checkin`。完整的类型签名、限制表、结果语义、弃用周期与机器可读契约清单见 [API v5 参考](api-v5.md)（[contracts/checkin-api-v5.json](contracts/checkin-api-v5.json)）。`describe()` 返回协议、API 版本、存储版本、能力、`capabilitiesSince`（每项能力首次出现的协议版本）和事件名称快照；`hasCapability(name)` 用于调用前检查单项能力。接入方应先检查 `isReady()` / `whenReady()`，再读取项目和事件；可通过 `capabilities` 和 `getCapabilityInfo()` 协商能力与本地写入边界，能力不存在时必须保留自己的离线流程。事件名称固定为 `checkin:item-created`、`checkin:item-updated`、`checkin:event-recorded`、`checkin:event-deleted`、`checkin:item-deleted`、`checkin:item-archived`、`checkin:suggestion-workflow-updated` 和 `checkin:analytics-updated`。`item-archived` 仅在自动归档成功后广播，携带归档项目的防御性快照；手动归档仍使用 `item-updated`，避免旧接入方重复处理。删除事件通过 `deletedEvents` 提供具体记录；建议事件仅包含规范化的建议 ID 与状态，不包含令牌或完整变更；分析事件仅表示只读快照已刷新。公共 API 不声明外部删除或建议写入能力。

## v5 新增能力（T-1275~T-1278）

- `items.query` → `queryItems({includeArchived?, archivedOnly?, kinds?, limit?})`：统一项目投影（归档语义二选一，`archivedOnly` 优先；类型过滤；限量默认 200/上限 1000）。
- `events.range.read` → `getEventsInRange({startDate, endDateExclusive}, {itemIds?, source?, includeSkips?, limit?})`：有界半开区间事件读（限量默认 1000/上限 5000，`truncated` 标注截断）。
- `events.record.batch` → `recordEventsBatch(inputs)`：幂等批量写入，单次持久化，结果与输入一一对应（`recorded`/`duplicate`/`discarded`/`blocked`/`rejected`），单批上限 200；`occurredAt` 缺省回退当前时刻并以 `usedFallbackTime` 标注。
- `metrics.read` → `getStreaks(itemIds?)`：当前连续与全历史最长连续（`computeLongestStreaks` 单一实现）。

externalRef 幂等前缀登记处新增 `obsidian21`（Obsidian Habit Tracker 21 迁入），完整规则见 [记录身份与多端合并规则](identity-and-merge.md)。
...
当前产品设置只开放 `siyuan-plugin-docktomato`（底栏番茄钟）这一外部提供方。小驴打卡通过 `window.__dockTomato.focus` v1 启动计时，监听持久化后的 `tomato:focus-session-completed`，并以 `docktomato:<sessionId>` 去重。旧偏好值 `plugin` 自动迁移为 `docktomato`；未来增加其他番茄钟时，应增加独立 provider 值和适配器 ID，不恢复“任取第一个已注册适配器”的不确定行为。
