...
能力集：`items.read` `items.query`（v5） `events.read` `events.range.read`（v5） `events.record.batch`（v5） `metrics.read`（v5） `occasions.read` `events.record` `occasions.complete` `summary.read` `summary.custom` `analytics.read` `export.json` `export.csv` `focus.adapters` `summary.providers` `suggestions.read` `integrations.events`。
