# 阻塞
- 2026-09-22 T-1389～T-1394：日历可见性研究已完成，无本地代码阻塞；真正落地仍依赖 Task Horizon 消费端接受项目×日期投影/能力协商，并需要双方 contract test 与思源桌面、页签、dock、Android、多窗口现场证据。当前不得修改 `task-horizon-v1.json` 来提前宣称能力；旧消费者若未升级不能保证项目级隐藏，T-1394 保持开放且不阻塞其他本地功能。
- 2026-09-22 T-1395：上游公开 API 双轨规划不阻塞本地 fallback 设计，但阻塞“默认稳定自动联动”的发布资格，直到合作插件维护者接受方案、API 合并、版本发布并完成真实思源桌面/移动/多窗口/重载验收。当前没有可安全臆造的第三方仓库路径或已发布接口；只准备本地契约草案、patch 草案和测试夹具，不创建/提交外部 PR。若上游未响应，保留能力探测、实验/仅观察和失败降级，禁止读取私有存储或把两条路径重复累计。
- 2026-09-20 T-1337~T-1340：回顾图形、智能体助手与全端精修通过最终生产包完整质量链、两组59布局/45内容、40定向、56日期断点、双色visual和标准全页矩阵，无新增本地阻塞。思源addAgentCapability只确认工具注册契约；真实宿主模型调用、批准能力及真机触控/安全区仍归B-007/T-023保持开放并暂跳过。复制提问与本地模拟SummaryProvider验证不冒充真实模型联调；未push/发版。
- 2026-09-20 T-1333~T-1336：回顾内容分层、筛选分页、按需分析与UI已通过完整质量链、两组49布局、两组45/45全展开审计、双色visual及标准宽度走查，无新增本地阻塞。真实思源WebView/触摸/软键盘/安全区/跨窗口仍归T-023/B-007，按既有策略保持开放且暂跳过；两份真宿主E2E仅更新导航路径并检查语法，不计作现场实跑。未push/发版。
- 2026-09-20 T-1323~T-1328：参考融合、全端连续布局、时长手动/专注双路径与自定义形式已通过五组最终浏览器矩阵、双色visual-qa；完整质量链状态见PROGRESS，无新增本地阻塞。真实思源WebView、触摸/软键盘/安全区/跨窗口及底栏番茄钟双插件联调仍归T-023/B-007，按既有策略保持开放但跳过，不阻塞本地交付。用户明确本轮可暂不考虑样式体积，D-246将CSS529713字节作为信息报告，历史520000硬线不适用于本轮；其余产物、功能及性能门禁保留。
- 2026-09-20 T-1317~T-1322：逐页信息层级/模式隔离/模板/维护页/双色配色细节已通过五组42+32+16浏览器矩阵、双色visual-qa及完整test:quality，无新增本地阻塞。指定主按钮8主题配色组合对比度已实测；仍不替代T-023/B-007真实WebView、触摸/软键盘/安全区和双插件联调。CSS511402字节接近520000硬线，后续视觉扩展优先清理现有样式，本轮未放宽门槛。
- 2026-09-20 T-1313~T-1316：展开交互/空态/卡片比例/回顾编辑精修及普通二值备注提交修复通过完整质量链、五组浏览器矩阵和双色visual-qa，无新增本地阻塞；CSS506247字节低于520000硬线，保留既有预算告警。T-023/B-007真实设备及宿主验证仍开放；指定文本对比度自动检查不代表全站或真实WebView验收。
- 2026-09-20 T-1309~T-1312：七页全端浏览器适配及白角修复完成，保留T-023/B-007真实宿主/触摸/系统键盘现场验收；没有新增本地代码阻塞。CSS498256字节进入已配置预算告警区，低于520000硬线，本轮未放宽预算。
- 2026-09-20 T-1306~T-1308：本地完整质量链、双主题视觉与 30 项跨端容量自动验收通过，无新增代码阻塞；真实客户端的顶栏观感、20–30 项触摸滚动/软键盘/安全区及双插件体验仍需 T-023/T-033/B-007 现场验证。浏览器假宿主证据不替代真机结果。
- 2026-09-20 T-1305：每日目标启动传参已完成本地自动化；当前环境按 E2E 标准路径未找到 SiYuan-Kernel，真实「60 分钟启动 → 10 分钟提前完成 → 实际值回写」仍归 B-007，不阻塞本地交付。
- 2026-09-19 执行策略：用户明确要求所有需本人操作/真机人工验证的步骤一律跳过，不得阻塞后续本地开发；对应任务保持开放以保留验收事实，但每轮直接越过，只有代码/自动化可推进部分继续实施。
- [ ] B-001 真实设备测试 | 影响 T-023 | 需要用户在思源桌面/手机端实际操作 | 暂跳过
- 验收入口：按 `docs/integration-smoke-checklist.md` 执行手机、页签、dock 与番茄钟双窗口场景；回传设备尺寸、焦点/滚动结果和异常截图后即可继续修复。
- [x] B-002 宽度走查浏览器环境 | 影响 T-032 的 `width-walkthrough` 证据 | 初始 Playwright Chromium 缺失 | 已改用系统 Chrome 重跑，width walkthrough 与无障碍审计均通过

- B-003 migration audit persistence deferred while restore mutation path is being isolated; preview/version reporting is complete and no data mutation was changed.

B-003 resolved: migration audit persistence is now isolated after successful store persistence; audit-save failure follows existing restore error handling.

- B-004 audit readability formatting deferred after a failed encoding-safe edit; raw audit JSON remains intact and no functional behavior changed.

- [x] B-005 CSS 体积预算超标 | 影响 `test:ui` 尾部与 `check:release`（`tests/release-assets.test.cjs:27`）| 未发布的 dock 宽度打磨提交使 dist CSS 增至 294502 字节，超出 283000 预算（T-109 基线 262270 + 8%）；同工具链下 v9.6.1 实测 264920 字节，+29582（+11.2%）全部来自未发布工作，非环境差异 | 已处置（2026-09-14）
- 处置记录：全量类名核查（419 个 class 全部在源码有引用）确认无 D-051 意义上的死样式，按选项②上调预算至 318000（新基线 294502 + 8% 余量），T-109 注释同步；四档样式合并精简立项为 15.0-A，完成后下修基线。`pnpm run test:quality` 已于 2026-09-14 全链复绿（exit 0）。
- 后续更新（2026-09-16）：D-089 已替代迁移期间的 380KB 硬线，现采用 318KB 软线、420KB 告警线与 450KB 硬阻断线；最近一次生产 CSS 为 404,587 bytes，完整质量链与跨宽度视觉回归通过，B-005 继续保持关闭。

- [x] B-006 真机截图核对（T-023 首批）| 用户提供 4 张手机端截图 | 逐项复现于 scripts/visual-probe.cjs 并修复 7 类布局问题 + 1 项文案门控；守门入 mobile-release-quality | 已修复（2026-09-14）
- 备注：截图中右下紫色悬浮钮为思源移动端自带侧栏钮（宿主 UI），插件侧以列表底部避让空间处理；打卡主按钮较 v9.6.1 略窄是为给操作图标让位的取舍。

- [ ] B-007 真实宿主复核 | 影响 T-023/T-129 | 自动化浏览器无法证明思源桌面/移动端页签、dock、安全区、软键盘及番茄钟双窗口行为 | 发布说明保留限制，待用户现场证据；不阻塞 GitHub v12.0.0 发布
- 本轮更新（2026-09-16）：T-962~T-974 已增加自动化布局、真实滚轮、滚动同步、命中与多宽度证据，但这些仍不能替代真实思源宿主；需重点复核手机软键盘下保存栏、展开模板后的触摸滚动、dock 320px 实际宽度、安全区及番茄钟双窗口。B-007 保持开放，不阻塞本地阶段提交。
- 本轮补充（2026-09-16）：T-975~T-979 已把跨日事件、完成区结构、pending-only 多 surface 预检和局部刷新选择器纳入自动化守门，但仍不能证明真实宿主中的焦点、滚动记忆及番茄钟双窗口时序；T-129 继续等待用户手机/页签/dock 现场记录，B-007 保持开放。
- 集市状态（2026-09-17）：官方 Bazaar PR #2248 已合并（2026-09-16 16:32 UTC，PR Check 通过），插件进入官方集市；该包级校验不覆盖真实宿主交互，B-007 继续开放。
- 番茄钟状态（2026-09-17）：Dock Tomato 兼容代码与 PR 草案已在本地完成自动化验证，但按用户要求尚未创建 PR；真实双插件、双窗口、移动端与跨设备完成时序仍归 B-007，需在用户审阅并由上游发布兼容版本后现场验收。
- 13.0 补充（2026-09-17）：T-988~T-994 已自动化覆盖提供方诊断、错误翻译、completion 伪造/重复/乱序、项目删除归档、映射漂移和写失败；真实插件热重载、双窗口并发及上游实际事件时序仍只能在兼容版本安装后验证，B-007 保持开放。
- 13.0 第三批（2026-09-17）：T-995~T-997 已覆盖诊断跨重载恢复、损坏数据裁剪及安全导出；`onDataChanged` 路径可重新载入，但真实思源多窗口是否对该独立数据文件稳定触发通知仍需 B-007 现场验证，不据自动化结果宣称双窗口实时一致。
- 13.0 第四批（2026-09-17）：T-998~T-1000 已自动化覆盖提供方状态抛错、恶意 getter、释放轮询和卸载失效；真实 Dock Tomato 的 ended/completed 事件顺序及宿主暂停恢复仍需兼容版本安装后的 B-007 验收。
- 13.0 第五批（2026-09-17）：T-1001~T-1003 已用可执行假宿主覆盖完整 bridge 行为和卸载清理；该 harness 不模拟真实思源跨 WebView 事件传递，因此 B-007 的双窗口现场门槛保持不变。
- 13.0 第六批（2026-09-17）：T-1004~T-1006 已覆盖写入空返回、抛错、同 session 重试及卸载后迟到结果；真实磁盘失败、插件同时卸载和上游重发策略仍需 B-007 联调确认。
- 13.0 第七批（2026-09-17）：T-1007~T-1009 已修复并验证单进程并发重放竞态；不同 WebView 各自拥有内存锁，跨窗口最终仍依赖主 store 的 externalRef/存储锁，需 B-007 真宿主并发完成验证。
- 13.0 第八批（2026-09-17）：T-1010~T-1012 已覆盖损坏历史 getter 和 25 个持久身份重放；真实跨设备同步延迟下两个窗口同时看不到 externalRef 的时间窗仍需 B-007 与主存储锁共同验证。
- 13.0 第九批（2026-09-17）：T-1013~T-1015 已减少历史幂等扫描分配但仍保持 O(n) 完整扫描；十万级索引属于 14.0 数据内核，当前不以截断历史换取错误的重复记账风险。
- 13.0 第十批（2026-09-17）：T-1016~T-1018 已自动化覆盖 availability 风暴与 facade 替换/停用/恢复；真实思源中两个插件加载先后、热重载窗口和宿主事件投递仍需 B-007 现场确认。
- 13.0 第十一批（2026-09-17）：T-1019~T-1021 已验证约5秒有界释放轮询及耗尽恢复；真实 Dock Tomato 状态落盘延迟是否超过该窗口仍需 B-007 联调观察，超时不会冒险提前释放。
- 13.0 第十二批（2026-09-17）：T-1022~T-1024 已验证20个失败 session 的成功重试与诊断解决；真实上游是否会对失败完成事件重新发送仍取决于兼容契约和 B-007 联调。
- 13.0 第十三批（2026-09-17）：T-1025~T-1027 已覆盖乱序与同时间诊断恢复；真实多窗口同时写诊断的最终覆盖策略仍依赖思源数据通知行为，继续由 B-007 验证。
- 13.0 第十四批（2026-09-17）：T-1028~T-1030 已覆盖运行期重复诊断折叠与计数；跨窗口各自折叠后写回的计数合并仍受存储通知时序影响，B-007 保持开放。
- 13.0 第十五批（2026-09-17）：T-1031~T-1033 已让载入时的重复行确定性归并并保留合计次数，但两个窗口在未收到彼此数据通知前同时覆盖文件的最后写入竞争仍需 B-007 真宿主验证。
- 13.0 第十六批（2026-09-17）：T-1034~T-1036 已覆盖诊断导出的恶意 getter、损坏时间和有界能力扫描；真实浏览器下载权限、文件保存提示与移动端分享行为仍属于 B-007 宿主验收。
- 13.0 第十七批（2026-09-17）：T-1037~T-1039 已确保外部专注启动上下文可被 completion 无损匹配；真实 Dock Tomato 对最大长度 context 的保存与事件回传仍需 B-007 联调验证。
- 13.0 第十八批（2026-09-17）：T-1040~T-1042 已消除 sessionId/recordId 截断碰撞并统一历史幂等边界；真实上游身份字符集与最大长度仍需 B-007 联调确认。
- 13.0 第十九批（2026-09-17）：T-1043~T-1045 已让实时 provider 检测与诊断导出共享安全版本/能力投影；真实上游热重载期间 facade 字段变化仍需 B-007 现场观察。
- 13.0 第二十批（2026-09-17）：T-1046~T-1048 已统一 runtime status、completion、历史幂等和诊断关联键的无损身份规则；真实上游是否始终提供无空白身份仍需 B-007 联调确认。
- 13.0 第二十一批（2026-09-17）：T-1049~T-1051 已限制损坏诊断数组的 getter 与超大稀疏扫描风险；真实多窗口产生的正常文件仍远低于512项，覆盖写竞争继续由 B-007 验证。
- 13.0 第二十二批（2026-09-17）：T-1052~T-1054 已在 JSON.parse 前限制诊断文本为512KiB；思源 loadData 在真实损坏大文件下的读取行为仍属于 B-007 宿主范围。
- 13.0 第二十三批（2026-09-17）：T-1055~T-1057 已隔离 completion 项目数组与字段访问器；真实主 store 项目规模和跨窗口变更时序仍由 B-007/14.0 性能工作覆盖。
- 13.0 第二十四批（2026-09-17）：T-1058~T-1060 已要求 completion 原样回传启动上下文并禁止时长隐式转换；真实 Dock Tomato 事件 payload 是否完全符合该公共契约仍需 B-007 联调。
- 13.0 第二十五批（2026-09-17）：T-1061~T-1063 已隔离持久历史数组的索引/迭代器访问器且保持完整扫描；真实十万级历史性能继续按14.0数据索引路线处理。
- 13.0 第二十六批（2026-09-17）：T-1064~T-1066 已合并同一在途窗口的结束释放请求；真实 Dock Tomato 是否会同时广播 completed/ended 及其先后顺序仍需 B-007 联调确认。
- T-105 第三十一批（2026-09-17）：设备能力样式已统一到组件层并通过自动宽度走查；真实 iOS/Android 安全区、系统打印预览和减弱动态偏好仍需 T-023 真机验收。
- T-105 第三十二批（2026-09-17）：legacy 生产引用已完全退役且自动宽度走查无溢出；模板管理器、打印和独立双色板的真实宿主观感仍并入 T-023/T-033 真机验收，不阻塞代码退役完成。
- 番茄钟状态（2026-09-19 晚）：PR #5 评审修复方案落地为 T-1266~T-1268/D-235~D-237——小飞驴侧启动误暂停、会话归属、可用性解绑、完成回写收件箱（completedAt 时钟/幂等/重试/atMost/skip）全部完成；对提供方的要求（start 返回最终 sessionId、pause-session 原子暂停、available detail、completedAt 必需、会话累计时长、并发互斥、默认关闭开关）已写入 docs/docktomato-integration-plan.md 契约节，待转交上游 PR 修订。真实宿主双插件联调仍归 B-007。
