...
- [ ] T-1346 UI 错乱清查与修复
  ...
  - 进度（2026-09-20，随模板一期）：宽度走查（Edge）浅深双主题 42 页面+32 交互+8 对比度+10 混合+16 长内容全过，含编辑器模板披露高度与芯片点击断言；模板区改动（分批/最近使用/预览摘要）无溢出回归。
  - 发现 1：pluginLanguage 设置未接线——setPluginLanguage 无运行时调用方，英文界面不可达（字典仅测试层维护）。建议 v18 内接线设置项并补英文布局走查后再宣布 EN 支持；接线前英文 UI 溢出风险不构成用户可见问题。
  - 发现 2：模板区分批显示与最近使用行已由 template-gallery 结构门禁覆盖，运行时（浏览器真实点击展开器/套用后置顶）断言待补入走查脚本。
