...
- [ ] T-1360 建议执行范围扩展：排期/目标字段建议执行（差异预览、before 基线校验、冲突跳过、可撤销不变）；历史事件永不在范围。注意：字段白名单已含 target/unit/group/timeSlot/tomatoMode，缺 schedule 深比较与本地建议生成器扩展。
...
- [ ] T-1346 UI 错乱清查与修复
  ...
  - 剩余：真机显示缩放（80%/110%）走查变量、dock 实宽 280px 场景——随 T-1344 真机窗口一并执行。
