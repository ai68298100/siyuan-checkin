...
## v18.6.0 候选冻结（2026-09-26，待用户确认发版）

- [x] v18.6.0 发版准备（版本四方 18.5.0→18.6.0、changelog、发布说明含 SHA 回填、README 要点块、R-REL-CHECK）——本地 test:quality EXIT=0（182 文件），Release assets v18.6.0 检查通过。**等用户明确「发版」指令后运行 `node scripts/release.cjs 18.6.0`**（脚本将 push main + tag + GitHub Release，属外发动作，按 D-272/AGENTS 禁止自动 push 纪律需显式授权）。
