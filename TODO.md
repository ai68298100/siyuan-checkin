...
- [ ] T-1403 健康数据快捷指令模板与文档（C 类，评估卡 4）：iOS 步数/体重快捷指令官方模板 + 内网 push 接入文档（内核地址/token 配置/本地网络权限/失败静默说明）+ 同日重放幂等说明（recordEvent 对同 source+externalRef 重放返回已有事件）；公开 API 零代码。真机模板验证归 T-1388 真机窗口。
