...
- [ ] T-1383 外部时长来源契约设计：来源描述符、能力协商、匿名身份、阈值与失败诊断；先补设计和契约测试，不接真实写入。
- [ ] T-1384 思阅适配器 MVP：消费阅读生命周期事件，完成片段结算、跨日、重载恢复、每日一次幂等写入；opt-in，真实桌面/移动端验收后再发布。
- [ ] T-1385 思播适配器评估与实现：优先等待/验证公开事件契约；只有 controller 轮询时先保留实验/仅观察模式，不默认自动写入。
