...
## 后续调研计划（2026-09-22 登记，先调研再定开发范围）

计划与交付标准见 [间隔打卡、文档绑定、每日日记与同类产品调研](docs/roadmap-checkin-research-2026-09.md)。本轮只登记计划与初步资料，以下研究任务均保持开放；不预定算法、版本或上线日期，不直接启动功能开发。执行顺序：T-1377 建立对照资料 → T-1374～T-1376 专题研究 → T-1378 归纳决策。延续用户选择偏好：有充分依据时自动采用推荐并记录理由，不要求用户逐项选择。
...
- [ ] T-1378 调研归纳与开发范围决策
  - 依赖 T-1374～T-1377 的研究产物；逐项给出做/延后/不做及证据，明确 MVP、非目标、数据/API/隐私影响、验收与回退方案，再拆正式开发批次。
  - 状态：未开始；本计划和初步来源检索不等同于完成充分调研，不继承为功能开发完成。
