...
api.safeAgentCall(() => "ok").then((result) => { assert.deepEqual(result, {ok: true, value: "ok"}); return api.safeAgentCall(() => { throw new Error("offline"); }); }).then((result) => { assert.equal(result.offline, true); assert.equal(result.error, "offline"); return api.safeAgentCall(() => new Promise((resolve) => setTimeout(() => resolve("late"), 20)), 1); }).then((result) => { assert.equal(result.offline, true); assert.equal(result.error, "智能体调用超时"); console.log("Integration ecosystem checks passed."); fs.rmSync(path.dirname(output), {recursive: true, force: true}); });
