...
for (const filename of ["types.ts", "record-step.ts", "quota.ts", "rules.ts", "date-keys.ts", "model.ts", "api-contract.ts", "ecosystem.ts"]) {
...
const manifest = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "docs", "contracts", "task-horizon-v1.json"), "utf8"));
const item = {
    id: "task-checkin",
    name: "任务打卡",
    icon: "✅",
