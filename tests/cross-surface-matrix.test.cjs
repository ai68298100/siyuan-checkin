...
assert.match(css, /topnav-tabs \{ display: flex;[\s\S]*scrollbar-gutter: stable;/);
assert.match(css, /topnav-action:focus-visible \{ outline: 2px solid var\(--lc-checkin-accent\);/);
assert.match(css, /\.lc-checkin--review \.lc-checkin__summary-stats[\s\S]*repeat\(4, minmax\(0, 1fr\)\)/);
assert.match(css, /summary-stats \.lc-checkin__analytics-badge \{ min-width: 0; max-width: 100%;/);
assert.match(css, /summary-stats > div \{ min-height: 56px; display: grid; place-content: center; overflow: hidden; \}/);
...
assert.match(css, /review-subnav \{ top: 58px; gap: 4px; overscroll-behavior: contain;/);
