...
export function renderSuggestionWorkflowPanel(state: SuggestionWorkflowState): string {
    ...
    const auditRows = state.audits.slice(-10).reverse().map((audit) => {
        const actionKey = audit.reason === "revert" ? "reverted" : audit.action;
        const reason = audit.reason && audit.reason !== "revert" ? `<small>${escapeHtml(t("agent.auditReason", {reason: audit.reason}))}</small>` : "";
        return `<li><span>${escapeHtml(t(`agent.audit.${actionKey}`))}</span><time datetime="${escapeHtml(audit.at)}">${escapeHtml(audit.at)}</time>${reason}</li>`;
    }).join("");
    const auditDetails = auditRows ? `<details class="lc-agent-suggestion-audits"><summary>${t("agent.auditDetails")}</summary><ol>${auditRows}</ol></details>` : "";
    return `<section class="lc-checkin__suggestion-workflow" data-suggestion-workflow="${escapeHtml(state.envelope.id)}" aria-label="${t("agent.workflowAria")}"><header><strong>${escapeHtml(state.envelope.title)}</strong><span class="is-${summary.status}" role="status" aria-live="polite" aria-label="${t("agent.workflowStatusAria", {status: statusLabel})}">${escapeHtml(statusLabel)}</span></header><p>${escapeHtml(state.envelope.reason || t("agent.noReason"))}</p><small>${t("agent.workflowAuditMeta", {consumed: summary.consumedTokens, audits: summary.audits, applied: audits.applied, rejected: audits.rejected})}</small><small class="lc-agent-suggestion-updated">${t("agent.workflowUpdatedAt", {date: escapeHtml(updatedAt)})}</small>${changeRows ? `<ul class="lc-agent-suggestion-changes">${changeRows}${hiddenNote}</ul>` : `<p class="lc-agent-suggestion-empty">${t("agent.noChanges")}</p>`}${auditDetails}<footer aria-label="${t("agent.workflowActionsAria")}">${actions}</footer></section>`;
}
