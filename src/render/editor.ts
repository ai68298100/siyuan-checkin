    ...
    return `<div class="lc-checkin lc-checkin--editor" data-appearance="${ctx.appearance}">
        ...
        return `<button class="lc-checkin__icon-option ${selectedIcon === icon ? "is-selected" : ""}" type="button" data-icon="${escapeHtml(icon)}" data-icon-search-text="${escapeHtml([icon, group.name, ...group.keywords, keywords].join(" "))}" aria-label="${t("editor.iconSelectAria", {label: t(`iconGroup.${group.id}`), icon})}" title="${escapeHtml(keywords || group.name)}">${escapeHtml(icon)}</button>`;
                        ...
                        <div class="lc-checkin__custom-icon" data-custom-icon-panel>
                            ...
                            ${ctx.customIconLibrary.length ? `<div class="lc-checkin__custom-library" data-custom-library><small>${t("editor.myIcons", {n: ctx.customIconLibrary.length})}</small><div class="lc-checkin__icon-grid">${ctx.customIconLibrary.map((icon) => `<button class="lc-checkin__icon-option" type="button" data-library-icon="${escapeHtml(icon)}" aria-label="${t("editor.useCustomIcon")}">${renderIconMarkup(icon)}</button>`).join("")}</div></div>` : ""}
                        </div>
                        </div>
                        </details>
