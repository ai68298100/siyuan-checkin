    ...
    const groups: Array<{id: string; label: string; body: string}> = [
        {
            ...
            body: `
                    ...
                    <label class="lc-checkin__settings-row"><span class="lc-checkin__settings-label"><span>左上角头像</span><small>可选择预设、输入文字，或上传照片（仅保存在本地）</small></span><span class="lc-checkin__settings-inline"><select data-setting-avatar aria-label="头像预设"><option value="check">✓ 勾选</option><option value="star">★ 星标</option><option value="horse">🐴 小驴</option><option value="leaf">🌿 绿叶</option><option value="sun">☀ 太阳</option><option value="target">🎯 目标</option></select><input data-setting-avatar-custom type="text" maxlength="8" value="${ctx.avatar.length > 2 || !["check","star","horse","leaf","sun","target"].includes(ctx.avatar) ? ctx.avatar.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\"/g, "&quot;") : ""}" placeholder="自定义" aria-label="自定义头像" /><input data-setting-avatar-file type="file" accept="image/png,image/jpeg,image/webp,image/gif" aria-label="上传头像照片" />${ctx.avatarImage ? `<button class="lc-checkin__text-button" type="button" data-setting-avatar-clear>清除照片</button>` : ""}</span></label>`,
        },
        {
            id: "today",
