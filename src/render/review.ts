...
export function renderReviewView(ctx: ReviewViewContext): string {
    ...
    const renderReport = (): string => {
        ...
        const providerButton = ctx.summaryProvidersCount ? `<button class="lc-checkin__text-button" type="button" data-action="generate-summary" ${ctx.summaryRefreshing ? "disabled aria-busy=\"true\"" : ""}>${ctx.summaryRefreshing ? t("review.agentRefreshing") : t("review.assistantGenerate")}</button>` : `<details class="review-assistant-provider-help"><summary>${t("review.assistantProviderHow")}</summary><p>${t("review.assistantProviderHowHint")}</p><code>window.siyuanCheckin.registerSummaryProvider({ id, name, summarize })</code></details>`;
