...
export function renderReviewView(ctx: ReviewViewContext): string {
    ...
    const summaryAdvice = attentionSummaryItem ? `可以先为“${escapeHtml(attentionSummaryItem.name)}”降低一次目标，保持连续记录。` : "继续保持当前节奏，逐步增加稳定完成的项目。";
    const summaryHero = `<section class="lc-checkin__review-hero" aria-label="回顾摘要"><div><small>截至 ${escapeHtml(summary.endDate)}</small><h2>${summaryRate >= 80 ? "保持得很好" : summaryRate >= 50 ? "正在稳步推进" : "从今天开始"}</h2><p>本周期完成 ${summary.completedItems}/${summary.scheduledItems || 0} 项，记录 ${summary.totalEvents} 条${topSummaryItem ? ` · 最佳项目：${escapeHtml(topSummaryItem.name)}` : ""}</p>${attentionSummaryItem ? `<small class="lc-checkin__review-attention">建议关注：${escapeHtml(attentionSummaryItem.name)}（完成率 ${attentionSummaryItem.completionRate}%）</small>` : ""}<small class="lc-checkin__review-advice">行动建议：${summaryAdvice}</small><button class="lc-checkin__text-button" type="button" data-action="preview-agent-suggestion" data-suggestion-item="${escapeHtml(attentionSummaryItem?.name || "")}" data-suggestion-rate="${attentionSummaryItem?.completionRate ?? ""}">查看建议影响</button></div><span class="lc-checkin__review-hero-rate">${summaryRate}%</span></section>`;
    ...
    const providerButton = ctx.summaryProvidersCount
        ? `<div class="lc-checkin__summary-agent"><span>${t("review.agentConnected")} · 截止 ${escapeHtml(summary.endDate)}${ctx.analysisLastGeneratedAt ? ` · 上次更新 ${escapeHtml(ctx.analysisLastGeneratedAt)}` : ""}${ctx.analysisHistoryCount ? ` · 已保存 ${ctx.analysisHistoryCount} 版` : ""}</span><button class="lc-checkin__text-button" type="button" data-action="generate-summary">更新当前分析</button></div>`
    ...
    return `<div class="lc-checkin lc-checkin--review" data-appearance="${ctx.appearance}">
            ...
            ${summaryHero}
            <nav class="lc-checkin__review-subnav" aria-label="回顾栏目"><button type="button" data-review-jump="0">提醒</button><button type="button" data-review-jump="1">趋势</button><button type="button" data-review-jump="2">项目</button><button type="button" data-review-jump="3">记录</button><button type="button" data-review-jump="4">分类</button><button type="button" data-review-jump="5">成就</button><button type="button" data-review-jump="6">计划</button></nav>
            ...
            ${fold("projects", `项目汇总 ${countBadge(summary.items.length)}`, `<section class="lc-checkin__review-projects"><div class="lc-checkin__review-project-list">${projectRows}</div></section>`)}
            ${fold("log", `${t("review.foldLog")} · ${ctx.store.events.length} 条`, renderCheckinLogView(ctx.store.events, ctx.store.items))}
            ${groupBars ? fold("balance", `分类平衡 ${countBadge(groupBars.match(/lc-checkin__balance-row/g)?.length || 0)}`, `<section class="lc-checkin__balance" aria-label="${t("review.balanceTitle")}">${groupBars}</section>`) : ""}
