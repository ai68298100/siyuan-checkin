...
export function renderReviewView(ctx: ReviewViewContext): string {
    ...
    const summaryHero = `<section class="lc-checkin__review-hero" aria-label="回顾摘要"><div><small>截至 ${escapeHtml(summary.endDate)}</small><h2>${summaryRate >= 80 ? "保持得很好" : summaryRate >= 50 ? "正在稳步推进" : "从今天开始"}</h2><p>本周期完成 ${summary.completedItems}/${summary.scheduledItems || 0} 项，记录 ${summary.totalEvents} 条${topSummaryItem ? ` · 最佳项目：${escapeHtml(topSummaryItem.name)}` : ""}</p>${attentionSummaryItem ? `<small class="lc-checkin__review-attention">建议关注：${escapeHtml(attentionSummaryItem.name)}（完成率 ${attentionSummaryItem.completionRate}%）</small>` : ""}<small class="lc-checkin__review-advice">行动建议：${summaryAdvice}</small></div><span class="lc-checkin__review-hero-rate">${summaryRate}%</span></section>`;
