...
export interface ReviewViewContext {
    ...
    reportSource: string;
    /** T-1432 · R-A8 命名保存视图（可选：旧桩缺省按无视图处理）。 */
...
export function renderReviewView(ctx: ReviewViewContext): string {
    ...
    const reminderActionButtons = (entry: (typeof reminders)[number]): string => {
        ...
        return `<span class="lc-checkin__reminder-actions" role="group" aria-label="${escapeHtml(t("review.reminderActionsAria"))}"><button class="lc-checkin__reminder-action" type="button" data-reminder-action="snooze" data-reminder-id="${id}" aria-label="${escapeHtml(t("review.reminderSnoozeAria", {name: entry.title}))}">${t("review.reminderSnooze")}</button><button class="lc-checkin__reminder-action" type="button" data-reminder-action="defer" data-reminder-id="${id}" aria-label="${escapeHtml(t("review.reminderDeferAria", {name: entry.title}))}">${t("review.reminderDefer")}</button><button class="lc-checkin__reminder-action" type="button" data-reminder-action="skip" data-reminder-id="${id}" aria-label="${escapeHtml(t("review.reminderSkipAria", {name: entry.title}))}">${t("review.reminderSkip")}</button></span>`;
    };
    ...
    const renderReport = (): string => {
        ...
        const providerButton = ctx.summaryProvidersCount ? `<button class="lc-checkin__text-button" type="button" data-action="generate-summary" ${ctx.summaryRefreshing ? "disabled aria-busy=\"true\"" : ""}>${ctx.summaryRefreshing ? t("review.agentRefreshing") : t("review.assistantGenerate")}</button>` : `<p class="review-scope-note">${t("review.assistantProviderHowHint")}</p>`;
        const assistant = `<section class="review-assistant" aria-label="${t("review.assistantTitle")}" data-summary-refresh-state="${ctx.summaryRefreshing ? "loading" : ctx.summaryError ? "error" : "idle"}">
            ...
            <div class="review-assistant-actions"><button class="lc-checkin__small-button" type="button" data-action="copy-open-review-agent">${t("review.assistantCopyOpen")}</button><button class="lc-checkin__text-button" type="button" data-action="copy-review-prompt">${t("review.assistantCopy")}</button>${ctx.analysisHistoryCount ? `<button class="lc-checkin__text-button" type="button" data-action="view-analysis-history">${t("agent.historyTitle")}</button>` : ""}</div>
        ...
        /* T-1359：智能体项目草案卡——检查并保存进入普通编辑器，不直接写 store。 */
        const draftCards = (ctx.projectDrafts || []).map((draft, index) => `<div class="lc-checkin__review-draft" data-project-draft="${index}"><strong>${escapeHtml(draft.name)}</strong><span>${escapeHtml(summarizeProjectDraft(draft))}</span><button class="lc-checkin__text-button" type="button" data-action="edit-project-draft" data-draft-index="${index}" aria-label="${t("review.draftInspectAria", {name: draft.name})}">${t("review.draftInspect")}</button></div>`).join("");
        const draftsBlock = draftCards ? `<div class="lc-checkin__review-drafts" data-project-drafts><strong>${t("review.draftsTitle")}</strong>${draftCards}</div>` : "";
        return `${assistant}${generated}<div class="lc-checkin__review-guidance-disclosure">${summaryGuidance}${attentionSummaryItem && hasPeriodRecords ? `<div class="lc-checkin__review-hero-actions"><button class="lc-checkin__text-button" type="button" data-action="preview-agent-suggestion" data-suggestion-item-id="${escapeHtml(attentionSummaryItem.itemId)}" data-suggestion-item="${escapeHtml(attentionSummaryItem.name)}" data-suggestion-rate="${attentionSummaryItem.completionRate}">${t("review.heroPreview")}</button><button class="lc-checkin__text-button" type="button" data-action="preview-agent-schedule-suggestion" data-suggestion-item-id="${escapeHtml(attentionSummaryItem.itemId)}" data-suggestion-item="${escapeHtml(attentionSummaryItem.name)}" data-suggestion-rate="${attentionSummaryItem.completionRate}">${t("review.heroSchedulePreview")}</button></div>` : ""}</div>${suggestionPanel}${draftsBlock}`;
    };
    ...
    const reportSourceOptions = [["", "report.sourceAll"], ["manual", "source.manual"], ["tomato", "source.tomato"], ["api", "source.api"], ["import", "source.import"], ["sireader", "source.sireader"], ["siplayer", "source.siplayer"], ["weread", "source.weread"], ["yeguif", "source.yeguif"]] as const;
    ...
    const exportScopeOptions = [["", "review.exportScopeAll"], ["7", "review.exportScope7"], ["30", "review.exportScope30"], ["90", "review.exportScope90"], ["365", "review.exportScope365"]] as const;
    const exportScopeSelect = `<label class="lc-checkin__report-option"><span>${t("review.exportScopeLabel")}</span><select data-export-days aria-label="${t("review.exportScopeLabel")}">${exportScopeOptions.map(([value, key]) => `<option value="${value}">${escapeHtml(t(key))}</option>`).join("")}</select></label>`;
