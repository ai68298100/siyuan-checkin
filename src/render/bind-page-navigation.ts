...
export function bindPageNavigationHandlers(root: HTMLElement, host: BindPageNavigationHost): void {
    ...
    root.querySelector<HTMLElement>("[data-action='view-analysis-history']")?.addEventListener("click", (event) => { const button = event.currentTarget as HTMLElement; let rows: Array<any> = []; try { rows = JSON.parse(button.dataset.analysisHistory || "[]"); } catch { rows = []; } const safe = (value: unknown) => String(value ?? "").replace(/[&<>\"']/g, (c) => ({"&":"&amp;","<":"&lt;",">":"&gt;",'\"':"&quot;","'":"&#39;"}[c] || c)); const list = rows.map((row, index) => `<li><strong>#${index + 1} · ${safe(row.asOf)}</strong><span>${safe(row.range)} · ${safe(row.source)} · ${safe(row.generatedAt)}</span></li>`).join(""); const options = rows.map((_, index) => `<option value="${index}">版本 #${index + 1}</option>`).join(""); const dialog = new Dialog({title: "历史分析", content: `<div class="lc-checkin__agent-preview"><div class="lc-agent-compare-select"><label>基准版本<select data-analysis-base>${options}</select></label><label>对比版本<select data-analysis-target>${options}</select></label><button class="b3-button" type="button" data-analysis-compare>准备对比</button></div><ul class="lc-agent-suggestion-changes">${list || "<li>暂无历史分析</li>"}</ul><p data-analysis-compare-status>历史版本当前为只读浏览，选择两个版本后可查看差异。</p></div>`}); dialog.element.querySelector<HTMLElement>("[data-analysis-compare]")?.addEventListener("click", () => { const base = dialog.element.querySelector<HTMLSelectElement>("[data-analysis-base]")?.value; const target = dialog.element.querySelector<HTMLSelectElement>("[data-analysis-target]")?.value; const status = dialog.element.querySelector<HTMLElement>("[data-analysis-compare-status]"); if (!status) return; status.textContent = base === target ? "请选择两个不同的分析版本。" : "对比视图将在下一步展示两版分析正文差异。"; }); });
