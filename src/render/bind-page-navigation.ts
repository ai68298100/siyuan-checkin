...
export function bindPageNavigationHandlers(root: HTMLElement, host: BindPageNavigationHost): void {
    ...
    root.querySelector<HTMLElement>("[data-action='view-analysis-history']")?.addEventListener("click", (event) => { const button = event.currentTarget as HTMLElement; let rows: Array<any> = []; try { rows = JSON.parse(button.dataset.analysisHistory || "[]"); } catch { rows = []; } const safe = (value: unknown) => String(value ?? "").replace(/[&<>\"']/g, (c) => ({"&":"&amp;","<":"&lt;",">":"&gt;",'\"':"&quot;","'":"&#39;"}[c] || c)); const list = rows.map((row, index) => `<li><strong>#${index + 1} · ${safe(row.asOf)}</strong><span>${safe(row.range)} · ${safe(row.source)} · ${safe(row.generatedAt)}</span></li>`).join(""); new Dialog({title: "历史分析", content: `<div class="lc-checkin__agent-preview"><ul class="lc-agent-suggestion-changes">${list || "<li>暂无历史分析</li>"}</ul><p>历史版本当前为只读浏览，后续将支持前后结果对比。</p></div>`}); });
