...
export function createReminderMigrationConfirmationToken(decision: ReminderMigrationDecision): string { return `rmt-${decision.canAutoApply ? "auto" : "confirm"}-${decision.risk}-${decision.delta}`; }
export function verifyReminderMigrationConfirmationToken(decision: ReminderMigrationDecision, token: string): boolean { return token === createReminderMigrationConfirmationToken(decision); }
