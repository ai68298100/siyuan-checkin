...
export function validateReminderMigrationDecision(decision: ReminderMigrationDecision): boolean { return typeof decision.canAutoApply === "boolean" && (decision.risk === "none" || decision.risk === "review") && Number.isInteger(decision.delta) && typeof decision.message === "string" && decision.message.length <= 300; }
export function refreshReminderMigrationDecision(impact: ReminderMigrationImpact): ReminderMigrationDecision { return evaluateReminderMigration(impact); }
export function formatReminderMigrationDecision(decision: ReminderMigrationDecision): string { return `${decision.canAutoApply ? "可自动应用" : "需要确认"} · ${formatReminderMigrationDelta(decision.delta)} · 风险：${decision.risk === "review" ? "需复核" : "无"} · ${decision.message}`; }
export const REMINDER_CONFIRMATION_TOKEN_VERSION = 1;
export function createReminderMigrationConfirmationToken(decision: ReminderMigrationDecision, now = Date.now(), nonce = Math.random().toString(36).slice(2, 10)): string { return `rmtv${REMINDER_CONFIRMATION_TOKEN_VERSION}-${decision.canAutoApply ? "auto" : "confirm"}-${decision.risk}-${decision.delta}-${now}-${nonce}`; }
export function verifyReminderMigrationConfirmationToken(decision: ReminderMigrationDecision, token: string, now = Date.now(), ttlMs = 10 * 60 * 1000): boolean { const parts = token.split("-"); const nonce = parts.pop(); const issued = Number(parts.pop()); return Boolean(nonce) && Number.isFinite(issued) && now - issued >= 0 && now - issued <= ttlMs && token.startsWith(`rmtv${REMINDER_CONFIRMATION_TOKEN_VERSION}-${decision.canAutoApply ? "auto" : "confirm"}-${decision.risk}-${decision.delta}-`); }
export function parseReminderMigrationConfirmationToken(token: string): {version: number; mode: "auto" | "confirm"; risk: "none" | "review"; delta: number; issuedAt: number; nonce: string} | null { const parts = token.split("-"); if (parts.length !== 6 || !parts[0].startsWith("rmtv")) return null; const version = Number(parts[0].slice(4)); const mode = parts[1]; const risk = parts[2]; const delta = Number(parts[3]); const issuedAt = Number(parts[4]); const nonce = parts[5]; return Number.isInteger(version) && (mode === "auto" || mode === "confirm") && (risk === "none" || risk === "review") && Number.isInteger(delta) && Number.isFinite(issuedAt) && Boolean(nonce) ? {version, mode, risk, delta, issuedAt, nonce} : null; }
export function summarizeReminderMigrationToken(token: string): string { const parsed = parseReminderMigrationConfirmationToken(token); if (!parsed) return "无效迁移确认令牌"; return `令牌 v${parsed.version} · ${parsed.mode === "auto" ? "自动" : "确认"} · 风险 ${parsed.risk} · 差异 ${parsed.delta} · 签发 ${new Date(parsed.issuedAt).toISOString()}`; }
export type ReminderTokenAuditAction = "created" | "verified" | "rejected";
export interface ReminderTokenAudit { action: ReminderTokenAuditAction; tokenSummary: string; at: string; }
export function createReminderTokenAudit(action: ReminderTokenAuditAction, token: string, at = new Date().toISOString()): ReminderTokenAudit { return {action, tokenSummary: summarizeReminderMigrationToken(token), at}; }
export function normalizeReminderTokenAudits(value: unknown, limit = 100): ReminderTokenAudit[] { if (!Array.isArray(value)) return []; const max = Math.max(1, Math.min(500, Math.floor(limit))); return value.filter((entry): entry is ReminderTokenAudit => Boolean(entry && typeof entry === "object" && ["created", "verified", "rejected"].includes((entry as any).action) && typeof (entry as any).tokenSummary === "string" && (entry as any).tokenSummary.length <= 300 && typeof (entry as any).at === "string" && !Number.isNaN(Date.parse((entry as any).at)))).slice(-max); }
export function serializeReminderTokenAudits(audits: readonly ReminderTokenAudit[]): string { return JSON.stringify({version: 1, audits: normalizeReminderTokenAudits(audits)}); }
export function deserializeReminderTokenAudits(value: string): ReminderTokenAudit[] { try { const parsed = JSON.parse(value); return parsed?.version === 1 ? normalizeReminderTokenAudits(parsed.audits) : []; } catch { return []; } }
