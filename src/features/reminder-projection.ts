...
export function createReminderMigrationConfirmationToken(decision: ReminderMigrationDecision, now = Date.now()): string { return `rmt-${decision.canAutoApply ? "auto" : "confirm"}-${decision.risk}-${decision.delta}-${now}`; }
export function verifyReminderMigrationConfirmationToken(decision: ReminderMigrationDecision, token: string, now = Date.now(), ttlMs = 10 * 60 * 1000): boolean { const parts = token.split("-"); const issued = Number(parts.pop()); return Number.isFinite(issued) && now - issued >= 0 && now - issued <= ttlMs && token.startsWith(`rmt-${decision.canAutoApply ? "auto" : "confirm"}-${decision.risk}-${decision.delta}-`); }
