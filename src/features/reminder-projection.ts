...
export function createReminderMigrationConfirmationToken(decision: ReminderMigrationDecision, now = Date.now(), nonce = Math.random().toString(36).slice(2, 10)): string { return `rmt-${decision.canAutoApply ? "auto" : "confirm"}-${decision.risk}-${decision.delta}-${now}-${nonce}`; }
export function verifyReminderMigrationConfirmationToken(decision: ReminderMigrationDecision, token: string, now = Date.now(), ttlMs = 10 * 60 * 1000): boolean { const parts = token.split("-"); const nonce = parts.pop(); const issued = Number(parts.pop()); return Boolean(nonce) && Number.isFinite(issued) && now - issued >= 0 && now - issued <= ttlMs && token.startsWith(`rmt-${decision.canAutoApply ? "auto" : "confirm"}-${decision.risk}-${decision.delta}-`); }
