...
export function renderTemplateManager(store: readonly UserTemplate[], state: TemplateManagerState): string {
    ...
    return `<section class="lc-template-manager" aria-labelledby="template-manager-title"><header><div><span class="lc-checkin__eyebrow">我的模板</span><h2 id="template-manager-title">常用打卡模板</h2><p>保存自己的打卡方式，下次新建时直接套用。</p></div><button type="button" data-template-action="new" aria-label="新建模板">新建模板</button></header><label class="lc-template-manager__search"><span>搜索模板</span><input type="search" data-template-query value="${escapeHtml(state.query)}" placeholder="搜索名称、分组或备注" aria-controls="template-manager-list" /></label>${editing ? renderTemplateForm(editing) : ""}<div class="lc-template-manager__count" aria-live="polite">${templates.length} 个模板</div><div class="lc-template-manager__list" id="template-manager-list">${templates.length ? templates.map((template) => renderTemplateCard(template, state.confirmDeleteId)).join("") : `<p class="lc-template-manager__empty" role="status">没有匹配的模板</p>`}</div></section>`;
}
