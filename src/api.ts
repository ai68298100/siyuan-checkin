...
export function createCheckinApi(host: CheckinApiHost): CheckinApi {
    ...
    return {
        ...
        recordEvent: (input) => {
            ...
            const moment = captureActionMoment();
            /* T-1384/T-1385/T-1402：sireader/siplayer/weread 是内部适配器保留来源，公开 API 输入一律回落 api，防止伪造。 */
            const snapshot = input && typeof input === "object" ? {...input, ...(input.source === "sireader" || input.source === "siplayer" || input.source === "weread" ? {source: "api" as const} : {})} : input;
