...
[最新 Release](https://github.com/ai68298100/siyuan-checkin/releases/latest) · [完整变更记录](docs/releases/release-notes-18.1.0.md) · [问题反馈](https://github.com/ai68298100/siyuan-checkin/issues) · [开发文档](docs/)
...
本次小版本把外部数据来源正式接入打卡体系，并增强连续统计与笔记锚点体验。主存储 v3、最低思源版本 3.8.4 不变，公开 API v5 新增 `calendar.read` 能力（共 20 项），可直接从 18.0.x 升级。

- **外部来源统筹框架**：统一「登记→接入→结算→身份→治理」五段管道与四类接入渠道，新增来源不改动结算与身份层。
- **思阅阅读联动**（opt-in 默认关）：消费思阅公开阅读生命周期事件，有效阅读分钟达阈值后为绑定项目每日一次幂等打卡，可随时删除撤销。
- **健康数据收件箱**（opt-in 默认关）：iOS 快捷指令把步数/体重写入收件箱文档，插件自动记入打卡；附官方模板与接入指南。
- **Task Horizon 日历可见性**：项目级「在任务管理器日历中显示」开关 + 有界只读日历投影，隐藏只影响外部日历。
- **每日摘要驻留**（opt-in 默认关）：每日把当天汇总一行追加到绑定文档，外部工具经思源接口即可读取。
- **容错连续（maxGap）**：项目级漏打容错天数，缺口在范围内不断签；与跳过日正交。
- **热力图色阶自适应**：四级色阶按个人记录分布自动分级，低频用户同样有完整层次。
- **锚点体验**：跳转前重新解析归属文档（块移动自动跟随），不可达时明确提示并回落项目详情。

完整条目见 [18.1.0 变更记录](docs/v18.1.0-change-log.md) 和 [发布说明](docs/releases/release-notes-18.1.0.md)。

### 18.0.x 功能基础

研究收口与体验修复（回顾布局、中文输入、头像编辑、日记设置），以及来源统筹框架与 Task Horizon 日历可见性的前期登记。
