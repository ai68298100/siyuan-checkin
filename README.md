...
当前开发线：4.0 响应式工作台 UI 已完成代码、自动化验证和 SiYuan 兼容 mock 视觉 harness 验收；真实思源桌面端和移动端仍需人工确认。
...
| 日期事项 | 管理生日、纪念日、定时事项和提前提醒 |
| 设置与归档 | 调整显示偏好，恢复暂时隐藏的打卡项目 |
...
- 项目设置：名称、内置图标、自定义图片图标、目标、单位、分组、重要性、时间段，以及每天、工作日、每周指定日和自定义日频率。
- 类型化目标：按次数、按时长、按数量和自定义值提供对应的目标步进、单位快捷选项和记录输入。
- 常用模板：新建项目时可直接选择阅读、喝水、运动、深度工作、冥想等 24 个预设，再按自己的计划调整。
- 编辑器体验：模板支持搜索和分组筛选，内置图标支持关键词检索；安排、分组和频率等高级设置按需展开，长表单保存操作固定在底部。
- 自定义图标：支持上传图片、将 HTTPS 图片下载到插件数据中、导入 JSON/逐行图标库，并提供阿里巴巴 Iconfont 入口。图片和图标库会占用本地存储，界面会在保存前提示预计空间。
...
- 手机端导航：窄屏下提供今日、事项、历史、总结、复盘、归档、设置和新建入口，并适配安全区和触控尺寸。
...
编辑打卡项目时可以把“完成来源”设为“番茄钟插件”，并选择按累计分钟或完成番茄钟数量计入目标。这个设置只声明项目的计量方式：兼容的番茄钟插件仍需在会话完成后调用 `recordEvent()`，传入 `source: "tomato"` 和稳定的 `externalRef`；未安装番茄钟插件时，项目仍可继续手动记录。
...
支持 `addAgentCapability` 的思源版本会向内置智能体注册六项能力：读取日、周、月或自定义范围的 `checkin-summary-context`，列出项目配置的 `checkin-list-items`，读取单项完成率与趋势的 `checkin-item-insights`，读取日期事项的 `checkin-list-occasions`，以及在用户明确要求后记录一次打卡的 `checkin-record-event` 或处理日期事项的 `checkin-complete-occasion`。前四项只读，后两项单独声明本地写入并复用插件的并发锁与持久化队列。用户可以直接让思源智能体使用思源已经配置的模型和密钥，无需在小驴打卡中重复维护 API 配置。
