...
- **微信读书联动**（official-pull 渠道，opt-in 默认关）：经微信读书官方 Agent API 拉取——每日阅读分钟达阈值记一次、书架每读完一本记一次、每日笔记条数（划线+想法）写入绑定项目；官方 skill 契约逐条核对（readdata/shelf/book/notes），`weread:` 前缀幂等+墓碑，断网静默降级。Key 仅存本地偏好、不入导出，接入步骤见 [微信读书接入指南](docs/weread-integration.md)。
