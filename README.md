...
本插件是纯前端插件，不包含 `kernel.js`，因此不声明 `kernels`，也不启用 `publish.data`。打卡记录、恢复点、审计信息和用户设置均属于本地私有数据，不会通过思源发布服务公开。

在只读或发布环境中，插件不应执行写入、同步、智能体执行或外部记录操作；生态 API 的写入能力必须通过能力检查和运行时状态检查后才可调用。未来若增加内核能力，将单独引入内核构建配置，不与当前前端包混用。

## 11.0.0 更新说明

- 优化桌面端顶栏、设置页、事项页与回顾页的比例、间距、固定定位和键盘焦点反馈。
- 优化手机端底栏为五列等宽布局，新增打卡按钮居中，窄屏下不再撑宽页面。
- 回顾页日历、统计卡片、历史记录、年度热力图和页签导航支持窄屏自适应。
- 完善移动端模板、表单、触控、滚动和安全区适配。
- 发布前已通过类型检查、移动端、生态集成、性能、发布资源检查及生产构建。
...
当前版本：**11.0.0**。本版本收口提醒中心、移动端窄屏、桌面事项页与发布稳定性门禁，并保留智能体建议的安全确认、审计、撤销和只读生态协作能力。
