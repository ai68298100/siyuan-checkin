...
- **跨端 UI 收口**：继续优化桌面、移动端、页签与 dock 的空间利用、对齐、导航密度和窄宽度内容保护。
- **侧边栏导航**：窄 dock 底栏使用五个等分轨道，图标和文字稳定居中；长标签、卡片正文及打卡按钮可安全收缩，宽 dock 导航均匀利用可用宽度。
- **局部刷新安全**：跨日事件、完成区切换和操作节点变化采用安全的完整投影；同结构数值更新继续局部处理，避免多窗口只更新一部分或卡片留在错误分区。
- **可靠性守门**：补充 dock 几何与局部刷新回归测试，并完成多宽度浏览器走查、移动端测试、生态测试和性能测试。

完整条目见 [12.0.1 变更记录](docs/v12.0.1-change-log.md) 和 [GitHub 发布说明](release-notes-12.0.1.md)。
