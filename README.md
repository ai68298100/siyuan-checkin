...
[最新 Release](https://github.com/ai68298100/siyuan-checkin/releases/latest) · [完整变更记录](docs/releases/release-notes-18.0.3.md) · [问题反馈](https://github.com/ai68298100/siyuan-checkin/issues) · [开发文档](docs/)
...
本次补丁版本完成间隔复习、项目文档绑定、每日日记、同类产品与闪卡联动的研究收口，形成采用、延后和不做边界，并拆出后续小版本候选。产品行为、数据结构、公开 API 与最低思源版本均不变，可直接从 18.0.2 升级。

- 普通习惯保留现有固定间隔、配额和强度口径；知识复习仅在独立 review 模式中继续评估 FSRS。
- 文档绑定继续复用 `noteAnchor`；优先收口根文档打开、失效挂起与移动端体验。
- 日记先采用只读摘要与用户确认写入；自动每日写入继续等待隐私与真实宿主证据。
- 闪卡调度与评分归思源宿主；优先人工打开入口，不轮询到期队列、不自动记账或制卡。

完整条目见 [18.0.3 变更记录](docs/v18.0.3-change-log.md) 和 [发布说明](docs/releases/release-notes-18.0.3.md)。
