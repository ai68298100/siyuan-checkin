...
## D-251：间隔打卡、文档绑定与日记联动先调研后定范围（2026-09-22）

- 用户新增四个后续方向：艾宾浩斯或同类间隔打卡、每项绑定指定文档、每日日记联动、继续吸收同类插件/软件经验。本轮只登记研究计划 T-1374～T-1378；不改业务代码、不预定算法或发版承诺。
...
- 延续用户“选择自动采用推荐”的要求：资料充分后由代理记录推荐及做/延后/不做结论，不增加逐项确认流程。本次仅记录计划，不据此启动功能开发；下一轮续跑先完成研究和 T-1378 范围决策。旧文档的“待用户选择”措辞不应妨碍已授权的方案研究，也不代表可提前启用自动写入。
